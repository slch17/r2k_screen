...
end_date = pd.to_datetime('2025-02-01')    # End date   
target_date = pd.to_datetime("2018-12-20") ### THIS IS THE TARGET DATE FOR BACKTEST/ DATA COLLECTION
...
fundamental_df.to_csv('fundamental_data_2018.12.20.csv')
...
historical_df.to_csv('historical_data_2018.12.20.csv')
...
data_df[['total_assets_value','prior_total_assets','asset_growth','z_asset_growth']]

for symbol in universe:
    data = collect_data(symbol, target_date, fundamental_data, historical_data)
    data_df.loc[len(data_df)] = data

data_df['asset_growth'] = np.where(data_df['prior_total_assets'] == 0,np.nan,data_df['total_assets_value'] / data_df['prior_total_assets'] - 1)
...
data_df['gross_margin'] = np.where(data_df['ltm_revenue'] != 0,data_df['ltm_gross_profit'] / data_df['ltm_revenue'],np.nan)
...
data_df.to_csv('data_df_2018.12.20.csv', index=False)
...
for symbol in historical_data:
    ...
    if 'date' in df.columns:
        ...
        df['adjusted_close'] = df['adjusted_close'].ffill()
        df = df.reset_index()
...
cumul_return_df_3y = cumul_return_df.head(1095) #take only 3 years of data 1095=365*3
cumul_return_df_3y.to_csv('cumul_return_2018.12.20.csv')

import matplotlib.pyplot as plt
# Ensure 'date' is the index for the 3-year DataFrame
if 'date' in cumul_return_df_3y.columns:
    cumul_return_df_3y = cumul_return_df_3y.set_index('date')

plt.figure(figsize=(10, 6))
cumul_return_df_3y['total_return_return'].plot(label='Portfolio Total Return', linewidth=2, color='black')
cumul_return_df_3y['iwm_return'].plot(label='IWM Total Return', linewidth=2, color='blue')
