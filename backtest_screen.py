...
target_date = pd.to_datetime("2011-09-30") ### THIS IS THE TARGET DATE FOR BACKTEST/ DATA COLLECTION
...
data_df['ltm_revenue'] = data_df['ltm_revenue'].replace(0, np.nan)  # Replace 0 with NaN to avoid division by zero
...
data_df['symbol','gross_margin','ltm_gross_profit','ltm_revenue'] 
...
test_portfolio=portfolio_selection(data_df,0.05)
...
def price_history(test_portfolio, historical_data,target_date=target_date):
    ...
    for symbol in test_portfolio:
        ...
        price_history[symbol] = historical_df[['date', 'adjusted_close']].set_index('date')
...
cumul_return=(price_history(test_portfolio, historical_data))
cumul_return_df = pd.concat(x.values(), axis=1, keys=x.keys())

import matplotlib.pyplot as plt

plt.figure(figsize=(10, 6))

cumul_return_df[('AAPL', 'return')].plot(label='AAPL Return')
cumul_return_df[('MSFT', 'return')].plot(label='MSFT Return')
cumul_return_df['total_return'].plot(label='Portfolio Total Return', linewidth=2, color='black')

plt.title('Portfolio Total Return vs AAPL and MSFT')
plt.xlabel('Date')
