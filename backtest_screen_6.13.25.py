...
historical_data['ROCK']
historical_data_df = pd.DataFrame(historical_data['ROCK'])
historical_df = pd.DataFrame(historical_data['ROCK'])
historical_df['date'] = pd.to_datetime(historical_df['date'])
historical_df = historical_df[historical_df['date'] >= target_date]

price_history = {}

price_history['ROCK'] = historical_df[['date', 'adjusted_close']].set_index('date')
base_price= price_history['ROCK']['adjusted_close'].iloc[0]
price_history['ROCK']['return'] = price_history['ROCK']['adjusted_close'] / base_price
...
plt.title('Portfolio Total Return Over Time')
plt.xlabel('Date')
